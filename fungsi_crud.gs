...
//fungsi cari data toko
function caritoko(chtId, cmd) {
	var txt = '';
	var item = cmd.match(/caritoko@(.+)/i);
	var cekData = false;
	var rs = bacaData();
	for (var i = 0; i < rs.length; i++) {
		if (rs[i][4].toString().toLowerCase() == item[1]) {
			cekData = true;
		}
	}
	if (cekData == true) {
		for (var x = 0; x < rs.length; x++) {
			if (rs[x][4].toString().toLowerCase() == item[1]) {
              var emoji = '';
              if (rs[x][6]=='PROSES'){
                emoji = ' 🔄'; 
              }else if (rs[x][6]=='DONE'){
                emoji = ' ✅';
              }else if (rs[x][6]=='CETAK'){
                emoji = ' 🖨️';
              }
              txt = '📝 <b>Menampilkan Data : ' + rs[x][4] + '</b>\n' 
